...
int main(int argc, char** argv) {
...
#endif
#if BGR2GRAY
    cv::Mat outputimg, ocv_outputimg, ocv_resizeimg;
    inputimg = cv::imread(argv[1], 1);
    if (!inputimg.data) {
        return -1;
    }
    //outputimg.create(inputimg.rows, inputimg.cols, CV_8UC1);
    outputimg.create(800, 800, CV_8UC1);
    ocv_outputimg.create(inputimg.rows, inputimg.cols, CV_8UC1);
    ocv_resizeimg.create(800, 800, CV_8UC1);
    /////////////////////////////////////// CL //////////////////////////////////////////////
    int height = inputimg.rows;
    int width = inputimg.cols;
    ...
    cv::imwrite("ocv_out.jpg", ocv_outputimg);
    cv::imwrite("hls_threshold_out.jpg", outputimg);
    //absdiff(outputimg, ocv_outputimg, error_img0);
...
//    xf::cv::Mat<TYPE, NEWHEIGHT, NEWWIDTH, NPC_T> out_mat(800, 800);
//
//    xf::cv::Array2xfMat<OUTPUT_PTR_WIDTH, XF_8UC1, HEIGHT, WIDTH, NPC_T>(outputimg, in_mat );
//    xf::cv::resize<INTERPOLATION, TYPE, HEIGHT, WIDTH, NEWHEIGHT, NEWWIDTH, NPC_T, MAXDOWNSCALE>(in_mat, out_mat);
//    xf::cv::xfMat2Array<OUTPUT_PTR_WIDTH, XF_8UC1, HEIGHT, WIDTH, NPC_T>(out_mat, ocv_resizeimg);
//	
//	cv::imwrite("ocv_resize_out.jpg", ocv_resizeimg);
	
    


    //cv::Mat xray_image_edit;
    //resize_img.copyTo(xray_image_edit);

    ////1) eq the historgram to increase the contrast
    //equalizeHistogram(ocv_outputimg);

    //// 2) find best threshold value and generate a mask for segementation
    //int threshold = 10; // starting threshold
    //int components = 0; // current components in bit mask
    //int xray_area = 0; // area of white components in bitmask
    //cv::Mat xray_mask = cv::Mat::zeros(ocv_outputimg.cols, ocv_outputimg.rows, CV_8UC1);
    //while (threshold < 256)
    //{
    //    
    //    xf::cv::threshold(ocv_outputimg, xray_image_edit, threshold, 255, cv::THRESH_BINARY_INV);

    //    // 3.1) remove noise (salt and pepper noise) from 3)
    //    cv::erode(xray_image_edit, xray_image_edit, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)), cv::Point(-1, -1), 1);

    //    // 4.) apply edge detection for connected components
    //    cv::Mat xray_e;
    //    cv::Canny(xray_image_edit, xray_e, 0.3, 0.6);

    //    // 4.1) connect edges where noise destroied the connections
    //    cv::dilate(xray_e, xray_e, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(1, 2)), cv::Point(-1, -1), 2);
    //    cv::dilate(xray_e, xray_e, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2, 1)), cv::Point(-1, -1), 2);

    //    // mask for image segemenation
    //    cv::Mat xray_tmp_mask = cv::Mat::zeros(ocv_outputimg.cols, ocv_outputimg.rows, CV_8UC1);

    //    // 5) find contours on image and fill them with 255 (true in mask)
    //    std::vector<std::vector<cv::Point>> contours;
    //    std::vector<cv::Vec4i> hierachy;       
    //    // 5.1) contour line detection     
    //    cv::findContours(xray_e, contours, hierachy, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);

    //    // 5.2) contour hull filling mask
    //    int area = 0;
    //    for(int contour_index = 0; contour_index < contours.size(); contour_index++) {
    //            auto points = contours[contour_index];
    //            if(std::find_if(points.begin(), points.end(), [ocv_outputimg](cv::Point p) { return p.x == 0 || p.y == 0 || p.x == ocv_outputimg.cols - 1 || p.y == ocv_outputimg.rows - 1; }) == points.end()) 
    //            {
    //                    std::vector<std::vector<cv::Point>> hulls(contours.size());
    //                    cv::convexHull(points, hulls[0]);
    //                    cv::drawContours(xray_tmp_mask, hulls, 0, cv::Scalar(255), -1);
    //                    area += cv::contourArea(contours[contour_index]);
    //            }
    //    }

    //    // 6) remove small objects (noise) in bit mask
    //    cv::erode(xray_tmp_mask, xray_tmp_mask, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(5, 5)), cv::Point(-1, -1), 15);            
    //    cv::dilate(xray_tmp_mask, xray_tmp_mask, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(5, 5)), cv::Point(-1, -1), 15);

    //    // 7) count the componnets in the mask and make sure that the final image only contains 2x lung wings and the background component
    //    components = cv::connectedComponents(xray_tmp_mask, cv::Mat(xray_tmp_mask.size(), CV_32S));

    //    // only use this mask as result mask if it has at least the 3 required components and the area increase or is greater than the initial area (e.g. 0)
    //    if(components == 3 && xray_area < area) {
    //            cv::copyTo(xray_tmp_mask, xray_mask, cv::Mat());
    //            xray_area = area;
    //    }

    //    // 8) increase the threshold and start from 3) or return the best mask if the threshold limit has been reached
    //    threshold +=5;       
    //}
    //    // 9) apply the mask to the original image and return the sgemented image
    //cv::Mat xray_image_lungs = xray_image.clone();
    //cv::bitwise_and(xray_mask, xray_image, xray_image_lungs);

    //// 10) increase the contrast of the segmented image
    //equalizeHistogram(xray_image_lungs);

    //// 11 a) filter light spots for ground glass patterns
    //cv::Mat xray_lungs_mask;
    //cv::threshold(xray_image_lungs, xray_lungs_mask, 180, 255, cv::THRESH_BINARY);

    //// 11 b) combine increased contrast lung segments with original image
    //cv::Mat final_image;
    //cv::subtract(cv::Mat(xray_image.cols, xray_image.rows, CV_8UC1, cv::Scalar(255)), xray_image, final_image);
    //cv::subtract(final_image, xray_mask, final_image);
    //cv::addWeighted(final_image, 1.0, xray_image_lungs, 1.0, 0.0, xray_image_lungs);

    //// convert from gray to rgb for highlighting        
    //cv::cvtColor(xray_image, xray_image, cv::COLOR_GRAY2BGR, 0);
    //cv::cvtColor(xray_image_lungs, xray_image_lungs, cv::COLOR_GRAY2BGR, 0);       

    ///*
    //// 11 c) apply highlight mask on originial image
    //cv::cvtColor(xray_lungs_mask, xray_lungs_mask, cv::COLOR_GRAY2BGR, 0);
    //cv::Mat green_mask(xray_image.cols,xray_image.rows, CV_8UC3, cv::Scalar(0, 255, 0));
    //cv::bitwise_and(green_mask, xray_lungs_mask, xray_lungs_mask);
    //cv::addWeighted(xray_image_lungs, 1.0, xray_lungs_mask, 0.5, 0.0, xray_image_lungs);
    //*/

    //// 13) return the original image and the highlighted image for comparision
    //cv::Mat result = cv::Mat::zeros(xray_image.cols, xray_image.rows * 2, CV_8UC3);
    //xray_image.copyTo(result(cv::Rect(0, 0, xray_image.cols, xray_image.rows)));
    //xray_image_lungs.copyTo(result(cv::Rect(xray_image.cols, 0, xray_image.cols, xray_image.rows)));
